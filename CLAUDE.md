...
- Las reservas de regalos son secretas para el dueño del deseo. Nunca denormalizar
  estado de reserva sobre `wishlist_items`, ni crear vistas sin `security_invoker`.
  Leer `supabase/README.md` antes de tocar `gift_reservations`.
...
Antes de dar por terminado un cambio: `npm run lint` y `npx tsc --noEmit`.
